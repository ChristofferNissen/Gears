...
// RENDER OUR GEARS BELOW

// OUR modules
module test_compound_meshing_double_helix(){
    compound_meshing_double_helix ();
}

module test_meshing_double_helix_with_screw(){
    meshing_double_helix_with_screw ();
}
	...
	translate ([0,0,pitch_apex1+20])
	{
        ...
        difference(){
            union() {
                ...
                // screw fastener piece
                rotate([0,-(pitch_angle1+pitch_angle2),0])
                translate([0,0,-pitch_apex2-8])
                difference()
                {
                    cylinder(r=13, h=10);
                    cylinder(d=8.1, h=20);
            ...
            // screw hole
            translate([63.5,0,7.5])
            union() {
                rotate([90,0,0]) cube([5.7,  2.7,  6],true);
                translate([0,0,-6]) 
                rotate([0,0,0]) cylinder(d=3, h=20);
            }
        }
...
//			outside_circular_pitch=outside_circular_pitch);
//
//        rotate([90,(pitch_angle1+pitch_angle2),0])
//		translate([0,0,-pitch_apex2])
//		bevel_gear (
//			number_of_teeth=gear2_teeth,
//			cone_distance=cone_distance,
	...
	finish = -1)
{
    ...
    echo ("Teeth: ", number_of_teeth, "Diameter: ", outside_pitch_diameter);
	...
	echo ("Num Teeth:", number_of_teeth, " Pitch Angle:", pitch_angle, "Module:", number_of_teeth/outside_pitch_diameter);
	...
	flat=false)
{
	...
	echo ("Teeth:", number_of_teeth, " Pitch radius:", pitch_radius, "Module:", number_of_teeth/pitch_diameter);
	// Base Circle
...
module compound_meshing_double_helix ()
{
	test_double_helix_gear (teeth=33, circles=0, bore_diameter=22);

	mirror ([0,1,0])
	//translate ([101.111,0,0])
	translate([0, 0, -11])
    rotate([0, 180, 0])
    
    test_double_helix_gear (teeth=11,circles=0,bore_diameter=22);
    // insert space disc
    difference()
    {
        translate([0,0,-6])cylinder(r=17, h=11);
        translate([0,0,-7])cylinder(r=11, h=13);
    }
    translate([0,0,-2.5])difference()
    {
        translate([0,0,-6])cylinder(r=13, h=6);
        translate([0,0,-7])cylinder(r=9.5, h=13);
    }    
}

module meshing_double_helix_with_screw ()
{
	test_double_helix_gear_with_screw (teeth=33, circles=8, circular_pitch=700);
//
	mirror ([0,1,0])
	translate ([87.5556,0,0]) // pitch 700
    //translate([48.888, 0, 0]) // pitch 400
    
    test_double_helix_gear_with_screw (teeth=11,circles=0,circular_pitch=700, bore_diameter=6);
    
}

module meshing_double_helix ()
{
	test_double_helix_gear (teeth=33, circles=8, circular_pitch=700);

	mirror ([0,1,0])
	translate ([87.5556,0,0]) // pitch 700
    //translate([48.888, 0, 0]) // pitch 400
    
    test_double_helix_gear (teeth=11,circles=0,circular_pitch=700);
    ...
    circular_pitch=700)
{
	//double helical gear
	{
		...
		gear (number_of_teeth=teeth,
			...
			hub_diameter=15,
			bore_diameter= bore_diameter, // same as bearing
			circles=circles,
			twist=twist/teeth);
...
module test_double_helix_gear_with_screw (
	teeth=17,
	circles=8,
    bore_diameter=8,
    circular_pitch=700)
	...
	//double helical gear
	{
        ...
        difference() {
            difference(){
                ...
                // cylinder (for screw to go through)
                translate([0,0,8.5])
                rotate([90,0,0])
                cylinder(d=3, h=50);
